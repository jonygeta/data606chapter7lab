...
download.file("http://www.openintro.org/stat/data/mlb11.RData","more/mlb11.RData")
...
Answer: I will use scatterplot to display relationship between runs and one of other nnumerical variable. 
```{r}
plot(mlb11$runs, mlb11$at_bats)
```

Yes. The relationsip seems linear. Yes. I will be comfortable using the variable at_bats as to predict the number of runs.
...
Answer: The relationshp between this to variable is linear, postive and moderately strong.
...
After serverals runs the values I can get are 123721.9,394354.3, 168982.8, 182855.8,152304.1. The smallest was 123721.9. It shows a better fitted line than the other neighbours.
...
Answer:
```{r}
fit <- lm(mlb11$runs~mlb11$homeruns)
summary(fit)
```

The regression equation is $Runs_{i} = 415.2389  + 1.8345 * homeruns_{i}$ 

The slope tells us that the success of a team and it's home runs are positively related. For 1 unit increase in homeruns the success of a teams increases by 1.8345 unit.
...
The predicted run will be 728.3166. 

```{r}
mlb11[mlb11$at_bats==5579,"runs"] - 728.3166
```

It's higher than the actural value of 713 runs. The model overestimates ~15 runs and the residual is -15.3166.
...
Answer:
There is no apparent pattern in the residual plot. Which indicates a linear model is appropriate.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    normal residuals condition appear to be meet
Answer: Based on the histogram and the normal probability plot normality of residuals holds.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
  ...
  Answer: Based on the plot in (1) constant variability condition appears to be met.
* * *
...
Answer: I've choosen runs and at_bats. 
```{r}
plot_ss(mlb11$runs, mlb11$wins)
```

At glance there seems to be a linear relationshipb between them. 
...
This model performs worst than the model with at_bats variable . Model with at_bats has r squared 0.6266 but this model with wins has r squared value 0.361. wins preditcs runs worst than at_bats.
...
fit <- lm(mlb11$runs~mlb11$homeruns)
summary(fit)
fit <- lm(mlb11$runs~mlb11$bat_avg)
summary(fit)
fit <- lm(mlb11$runs~mlb11$strikeouts)
summary(fit)
fit <- lm(mlb11$runs~mlb11$new_onbase)
summary(fit)
```

Out of five variables choosen hits performs better in predicting runs with a r squared value of 0.6419.
...
fit <- lm(mlb11$runs~mlb11$new_onbase)
summary(fit)
fit <- lm(mlb11$runs~mlb11$new_slug)
summary(fit)
fit <- lm(mlb11$runs~mlb11$new_obs)
summary(fit)
```

They seems to be more effective at predicting runs that the old variables with r square value more than 80% for each one than the pervious fives less than 64% r squared value.
...
```
    ...
    From the residual vs fitted plot and normal quantile plot we can see that the residuals are normally distributed and the assumptions are satisfied.
